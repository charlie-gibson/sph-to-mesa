...
with open(f"bestfit{starnum}_hse.sph", "w") as f:
    ...
    initial_guess = usph[0]  # You may need to adjust this depending on your problem
    ...
    for index in range(len(ammrho)):
        ...
        u[index] = result.x[0]        
        initial_guess = u[index] # use this u[index] as the initial guess for the next shell
...
def hydrostatic_equilibrium(eosfile):
    eos_data = read_or_load_eos(eosfile)
