...
def read_eos(eosfile):
    # eosfile = "sph.eos_X0.00to0.75step0.05_with_entropy"
    ...
    with open(eosfile, "r") as f:
        ...
        for ix in range(numx):
            ...
            if (ix == 0):
                ...
                eostable = np.empty((numu,numrho,numx,4), dtype = float)
            ...
            for irho in range(numrho):
                for iu in range(numu):
                    ...
                    utable[iu] = inputs[1]
                    eostable[iu][irho][ix][:] = inputs[2:6]
...
def useeostable(eos_data, ucgs, rhocgs, xxx, which):
    ...
    #     which=3 returns specific entropy of the gas (excludes entropy of radiation)
    ...
    if 0 <= irho <= numrho - 2 and 0 <= iu <= numu - 2:
        ...
        if iu>=0 and iu<=numu-2:
            ...
            if which != 1:
                ...
                #     so if which != 1 the code is supposed to calclate pressure, temperature, or specific entropy.
                ...
                # entropy will NOT be extrapolated correctly
        ...
        if which == 1:
            ...
            print("*********WARNING: OUTSIDE THE TABLE BOUNDARY ON COLD END. DO NOT TRUST THE T or P or s",which,irho,iu)
            ...
            b = -q
            b2 = np.sqrt(aa)
            #print('aa=',aa,piece1,piece2,r,ucgs)
            if b2 == 0:
            ...
            if which == 0:
                ...
                ## Take a stab at the specific entropy, although it will probably be
                ## at best off by an additive constant
                #pgas=rhocgs*boltz*temperature/meanmu
                #prad=arad*temperature**4/3
                #beta1=pgas/(pgas+prad)
                #useeostable=1.5*boltz/meanmu*(np.log(boltz*temperature/(rhocgs**(2/3)*meanmu))+8/3*(1/beta1-1))

                #print('Check entropy')
                #s0 = eostable[0][irho][ix][3]
                #s1 = eostable[0][irho][ix + 1][3]
                #useeostable = (xlow * s1 + xhigh * s0) / stepx

                # maintain constant slope of s vs log u at fixed rho:
                iu = 0
                ulow = log10u - (utablefirst + iu * stepu)
                uhigh = utablefirst + (iu + 1) * stepu - log10u
                if numx > 1:
                    s0 = ulow * eostable[iu + 1][irho][ix][3] + uhigh * eostable[iu][irho][ix][3]
                    s1 = ulow * eostable[iu + 1][irho][ix + 1][3] + uhigh * eostable[iu][irho][ix + 1][3]
                    useeostable = (xlow * s1 + xhigh * s0) / (stepu * stepx)
                else:
                    useeostable = (ulow * eostable[iu + 1][irho][0][3] + uhigh * eostable[iu][irho][0][3]) / stepu
